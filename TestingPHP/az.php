<?php
    ...
    if(isset($_POST["offset"]) || isset($_POST["limit"])){
      ...
      // $active = $_POST["active"];
      // FOR PAGINATION
      $az = filter_var($_POST['az'], FILTER_SANITIZE_STRING);
          
          
      $sql = "SELECT LEFT(s_name, 1), pf_db.* FROM pf_db WHERE s_name LIKE '$az%' GROUP BY s_name LIMIT $offset, $limit";
      ...
      while ($row = mysqli_fetch_array($result)){
        ...
        echo            '<label class="name" id = "card_name" ><h5 class="flHighlight">'.$row["s_name"].',</h5>'.$row["fs_name"].'</label>';
        ...
        echo '<div class="norecord" id = "norecord" style = "font-size: 30px; width: 100%; height:200px; text-align: center;">Found 0 results for "'.$az.'"</div>';
      }  
      $conn->close();
    }       
?>