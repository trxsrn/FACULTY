<?php
    ...
    if(isset($_POST["offset"]) || isset($_POST["limit"])){
        ...
        // $active = $_POST["active"];
        // FOR PAGINATION
        $department = filter_var($_POST['d_select'],FILTER_SANITIZE_STRING);

            
        $sql = "SELECT  *From pf_db WHERE `d_department` = '$department' GROUP BY s_name LIMIT $offset, $limit ";
        ...
        if($trecords>0){
            while ($row = mysqli_fetch_array($result)){
                ...
                echo            '<label class="department" id = "card_department" style = "color: #000000; font-weight: bold;">'.$row["d_department"].'</label>';
